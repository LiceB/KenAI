...
# Carregar a imagem da pessoa conhecida
...
while True:
    ...
    for (x, y, w, h) in rosto:
        ...
        if len(encodingsRosto) > 0:
            comparacao = fr.compare_faces([encodingsPessoa], encodingsRosto[0])
            distancia = fr.face_distance([encodingsPessoa], encodingsRosto[0])

            # Exibe o resultado do reconhecimento na imagem da webcam
            if comparacao[0]:
                cv2.putText(frame, "Larissa", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
            else:
